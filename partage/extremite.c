...
int ext_out (char* port, int fd) {
  ...
  if (err < 0){
	...
	if ((socketServer = socket(resol->ai_family, resol->ai_socktype, resol->ai_protocol)) < 0) {
		perror("allocation of socket");
		exit(3);
	}
	printf("Number of socket serveur is %i.\n", socketServer);
    ...
    if (err < 0) fprintf(stderr,"client resolution (%i): %s\n", socketClient, gai_strerror(err));
		else printf("Connection client successfull: fd = %i /// ip = %s /// port = %s\n", socketClient, hotec, portc);

		while(1) {
		    int err = socket_to_tun(socketClient, fd);
        if(err == -1) fprintf(stderr, "Error redirection client to tunnel\n");
    }
	}
...
int ext_in(char* ipServ, char* port, int fdTun) {
  ...
  if (socket < 0) {
	...
	printf("Attempt connect to %s /// ip = %s on port %s \n", ipServ, ip, port);
